...
class CrawlTaskTests {
	...
	void testProcessesPageSuccessfully() throws Exception {
		...
		when(queue.poll(10, TimeUnit.SECONDS))
		...
		try (MockedStatic<Robots> robotsMock = mockStatic(Robots.class);
			...
			// Stub Robots
			robotsMock.when(() -> Robots.isAllowed("http://example.com"))
					.thenReturn(true);
			...
			jsoupMock.when(() -> Jsoup.connect("http://example.com"))
