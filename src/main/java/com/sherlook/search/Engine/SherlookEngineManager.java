...
public class SherlookEngineManager {
    ...
    public SherlookEngineManager(DatabaseHelper databaseHelper, Ranker ranker, Crawler crawler, Indexer indexer) {
    ...
    public void processPostCrawlTasks(){
        ...
        Future<Void> indexingFuture = executor.submit(() -> {
            indexer.index();
            return null;
        });
        Future<Void> pageRankFuture = executor.submit(() -> {
